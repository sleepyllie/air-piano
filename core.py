...
from playsound3 import playsound # -- may be deprecated soon!
import asyncio
# import pyaudio
...
async def play_note(keyevent : KeyEvent, time_val = 100, duration_val = 100, velocity_val = 60):
    
    if str(keyevent) in cache:
        print("using cache...")
        # dump_wav("out.wav", cache[str(keyevent)], sample_rate=44100, use_int16=True) # dump_wave is the bottleneck here...
        #playsound("out.wav")
        #stream.write(cache[str(keyevent)])
        return
    ...
    print(audio)
    cache[str(keyevent)] = audio
    #sd.play(audio, sps)
    
    #stream.write(cache[str(keyevent)])
    dump_wav(f"{len(cache)}.wav", audio, sample_rate=44100, use_int16=True)
    playsound("out.wav")